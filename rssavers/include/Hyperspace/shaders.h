...
"	float eyedotnorm = dot(eyevec, texnormal);\n"
"	vec3 reflectvec = (texnormal * (eyedotnorm * 2.0)) - eyevec;\n"
"	// reflected color\n"
...
"	color.rgb = 2.0 * mix(color.rgb * gl_Color.rgb, max(vec3(0.0, 0.0, 0.0), color + gl_Color.rgb - vec3(1.0, 1.0, 1.0)), 0.6);\n"
"	// depth cue\n"
...
"	// overbrighten and modulate with vertex color using a mix of regular multiplicative and subtractive alpha\n"
"	color.rgb = 2.0 * mix(color.rgb * gl_Color.rgb, max(vec3(0.0, 0.0, 0.0), color + gl_Color.rgb - vec3(1.0, 1.0, 1.0)), 0.5);\n"
"	// depth cue\n"
"	color = mix(gl_Fog.color, color, clamp((gl_Fog.end + vsPosition.z) * gl_Fog.scale, 0.0, 1.0));\n" 
"	gl_FragColor = color;\n"
"}\n"
